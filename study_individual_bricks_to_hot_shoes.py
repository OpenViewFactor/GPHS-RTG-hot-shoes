...
brick_levels = os.listdir(bricks_dir)
...
for level in brick_levels:
  all_bricks = os.listdir( os.path.join( bricks_dir , level ) )
  log_output = os.path.join( logs_dir , level )

  for brick in all_bricks:
    for shoe in all_hot_shoes:
      log_name = brick[:-4] + "--" + shoe[:-4]
      log_output_path = os.path.join( log_output , log_name )

      brick_path = os.path.join( bricks_dir , level , brick )
