function generate_individual_bricks(REFINE_SCALE, EXPORT_TOGGLE, GRAPH_TOGGLE, STEP_TYPE)
  arguments
    REFINE_SCALE int8 {mustBePositive, mustBeInteger} = 15
  ...
  % TODO add top face for row 1 (facing positive z)
  % TODO add bottom face for row 18 (facing negative z)

  x_y_plane_mesh = generate_rectangle( [ -GPHS_WIDTH_x/2 , -GPHS_DEPTH_y/2, 0;...
                                         GPHS_WIDTH_x/2 , -GPHS_DEPTH_y/2, 0;...
                                         GPHS_WIDTH_x/2 ,  GPHS_DEPTH_y/2, 0;...
                                        -GPHS_WIDTH_x/2 ,  GPHS_DEPTH_y/2, 0;] , REFINE_SCALE );  % facing pos z

  top_face_mesh = translateMesh(x_y_plane_mesh, [0,0,GPHS_STACK_COUNT/2 * GPHS_HEIGHT_z]);
  bottom_face_mesh = flipNormals(translateMesh(x_y_plane_mesh, [0,0,-GPHS_STACK_COUNT/2 * GPHS_HEIGHT_z]));

  top_brick_mesh = BRICK_meshes{1, 1};
  top_brick_mesh_points = [ top_brick_mesh.Points; top_face_mesh.Points ];
  top_brick_mesh_connectivity = [ top_brick_mesh.ConnectivityList; top_face_mesh.ConnectivityList + size(top_brick_mesh.Points,1) ];

  top_brick_mesh = triangulation(top_brick_mesh_connectivity, top_brick_mesh_points);

  bottom_brick_mesh = BRICK_meshes{GPHS_STACK_COUNT, 1};
  bottom_brick_mesh_points = [ bottom_brick_mesh.Points; bottom_face_mesh.Points ];
  bottom_brick_mesh_connectivity = [ bottom_brick_mesh.ConnectivityList; bottom_face_mesh.ConnectivityList + size(bottom_brick_mesh.Points,1) ];

  bottom_brick_mesh = triangulation(bottom_brick_mesh_connectivity, bottom_brick_mesh_points);

  BRICK_meshes{1, 1} = top_brick_mesh;
  BRICK_meshes{GPHS_STACK_COUNT, 1} = bottom_brick_mesh;

  %* --------------- EXPORT MESHES --------------- *%
  if EXPORT_TOGGLE
    for i = 1 : GPHS_STACK_COUNT
        stlwrite(BRICK_meshes{i, 1}, output_path + "GPHS_brick-row-" + num2str(i) + "-ref-" + num2str(REFINE_SCALE) + ".stl", "binary")
    end
  end
