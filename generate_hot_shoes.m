function generate_hot_shoes(REFINE_SCALE, EXPORT_TOGGLE, GRAPH_TOGGLE)
  arguments
    REFINE_SCALE int8 {mustBePositive, mustBeInteger} = 10
  ...
  if EXPORT_TOGGLE
    for i = 1 : SHOE_STACK_COUNT
        stlwrite(SHOE_meshes{i, 1}, output_path + "x_z_positive_y_face-row-" + num2str(i) + "-ref-" + num2str(REFINE_SCALE) + ".stl", "binary")
        stlwrite(SHOE_meshes{i, 2}, output_path + "left_diagonal_face-row-" + num2str(i) + '-ref-' + num2str(REFINE_SCALE) + ".stl", "binary")
        stlwrite(SHOE_meshes{i, 3}, output_path + "right_diagonal_face-row-" + num2str(i) + "-ref-" + num2str(REFINE_SCALE) + ".stl", "binary")
        stlwrite(SHOE_meshes{i, 4}, output_path + "y_z_positive_x_face-row-" + num2str(i) + "-ref-" + num2str(REFINE_SCALE) + ".stl", "binary")
    end
  end
